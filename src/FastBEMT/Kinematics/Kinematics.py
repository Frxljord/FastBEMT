...
class Kinematics:
    ...
    every blade section at every source time.
    ...
    def _compute(self) -> None:
        ...
        self.center_of_mass_shift_blade_frame = torch.stack(
            [
                self.com_shift_up,
        ...
        self.section_position_blade_frame = (
            torch.einsum(
                "sij,sj->si",
                self.blade_to_airfoil_rotation_matrix.transpose(-1, -2),
                self.section_position_airfoil_frame,
            )
            + self.center_of_mass_shift_blade_frame
        )
        self.section_position_global_frame = torch.einsum(
            "tbij,sj->tsbi",
            self.global_to_blade_rotation_matrix.transpose(-1, -2),
            self.section_position_blade_frame,
        ).contiguous()

        self.angular_velocity_vector = torch.zeros(
            (1, 1, 1, 3),
            dtype=self.dtype,
            device=self.device,
        )
        self.angular_velocity_vector[..., 0] = self.omega
        self.section_velocity_global_frame = torch.linalg.cross(
            self.angular_velocity_vector,
        ...
        self.section_velocity = self.section_velocity_global_frame
        self.section_acceleration = self.section_acceleration_global_frame
        self.section_jerk = self.section_jerk_global_frame
        ...
        self.vel_fixed = self.section_velocity_global_frame
        self.acc_fixed = self.section_acceleration_global_frame
        self.jerk_fixed = self.section_jerk_global_frame
