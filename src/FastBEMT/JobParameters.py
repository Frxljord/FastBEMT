...
class LowFidelityParameters:
    ...
    def __init__(
        ...
        self.src_times = torch.arange(0, self.num_obs_times, dtype=torch.float32, device=self.device) * self.dt
        self.src_times_one_rotation = torch.arange(0, self.num_obs_times_per_rev, dtype=torch.float32, device=self.device) * self.dt
        self.num_src_times = self.src_times.shape[0]
        self.blade_angles = 2.0 * np.pi / n_blades * torch.arange(n_blades, dtype=torch.float32, device=self.device)